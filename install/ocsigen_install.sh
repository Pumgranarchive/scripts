...
echo "yes"  | sudo apt-get install pkg-config
...
opam install -y type_conv ocamlify ocamlmod lwt menhir oasis react calendar optcomp ipaddr ocamlnet pcre cryptokit dbm camlzip ssl #ocaml-text
 eval `opam config env`

opam install -y deriving
...
opam install -y tyxml js_of_ocaml ocsigenserver eliom
